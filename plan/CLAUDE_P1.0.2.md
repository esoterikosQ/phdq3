...
4. **site-packages의 transformers 코드를 직접 편집하지 않는다.** HF 구현의
   ...
   **attention mask**: 가중치는 Meta 스택의 local block-causal로 사전학습된
   것이므로, HF 구현이 단순 causal이면 원본 마스크(window + EOS segment 격리)를
   patched/subclass에 구현하는 것이 원본 충실 경로다. 이때 window/EOS 의미는
   원본 환경 없이 end-to-end 검증이 불가하므로 수학적 정의 기반 단위 테스트가
   유일한 검증 수단이며, 정확한 마스크를 구현하지 않은 실행은
   `HF full-causal variant`로만 분류하고 main run에 쓰지 않는다.
...
- conda env: `phdq_blt_hf` — PyTorch stable **cu128 wheel** +
  `transformers==5.16.1` 고정(범위 지정 금지), xFormers 없음. cu128은
  itcerdo 5090(sm_120)과 neuron A100(sm_80)을 하나의 환경으로 커버한다.
- **노드 역할** (SLURM은 neuron에만 존재):
  - gsm/yellowstone(mac): 코드 작성·정적 테스트·결과 취합. CUDA 미설치.
  - itcerdo(5090, 일반 Linux): Phase A 호환성·parity·시간 실측 전용.
...
- itcerdo↔neuron 교차 비교는 정수 산출물(patch 경계, token ID) 완전 일치 +
  실수 산출물(entropy, logits) 허용 오차 원칙. parity 본판정은 동일 GPU에서.
...
```bash
# --- itcerdo (일반 Linux shell, SLURM 금지) ---
...
/usr/bin/time -v python blt_hf_checks/bench_compatibility.py \
  --model-id artifacts/converted/blt-1b-hf-own --local-files-only \
...
# --- gsm/yellowstone (mac, CPU) ---
pytest tests/test_hf_data_adapter.py tests/test_hf_manifest.py -q
python blt_hf_checks/analyze_data_lengths.py   # 9개 canonical split 길이 통계
```
